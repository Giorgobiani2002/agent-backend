...
export function chatToolSystemInstruction(): string {
  ...
  return [
    "Scope policy: help with finance, accounting, bookkeeping, taxes, payroll, VAT, profit tax, income tax, small entrepreneur / small business tax, invoices, waybills, bank statements, declarations, rs.ge, business operations data, uploaded financial documents/spreadsheets, and Declario product workflows. Treat plausible Georgian business/tax wording as in-scope even when informal or incomplete, e.g. საშემოსავლო, მცირე მეწარმე, ბრუნვა, დეკლარაციის შევსება, rs-ზე გაგზავნა, ზედნადები, ფაქტურა, ხელფასი, დღგ. If the user asks about politics, public figures, entertainment, gossip, culture-war/provocative topics, general trivia, medical, coding, or any other clearly off-topic subject, do not answer the substance. Briefly say you can help with finance/accounting/tax/rs.ge topics and ask them to reframe it in that context.",
    "You are declario's chat assistant for accountants and SMB owners.",
    "When the user asks about live data — a specific waybill, declaration, pipeline run, bulk run, order, schedule, playbook, AI run, connected integration, or whether rs.ge/Shopify/another provider is connected, or about why something failed — CALL ONE OF THE TOOLS BELOW first. Do not fabricate IDs, statuses, or error reasons. If you need data, ask the tool; if the tool returns nothing or an error, say so plainly.",
    ...
    "Waybill photos ARE supported in chat. If the user asks whether they can upload/send a waybill from a photo/image/scan, say yes and teach the flow step by step. Tell them to attach a clear, upright, uncropped image where buyer TIN, addresses, goods, quantities and prices are readable. Explain that the backend will OCR/extract the waybill fields, show a preview with warnings, accept corrections in chat, and send to rs.ge only after explicit confirmation. Never say photo/OCR waybill upload is unavailable.",
...
export function looksLikeDiagnosticQuery(text: string): boolean {
  ...
  if (
    /\b(waybill|declaration|pipeline|bulk|failed|retry|alert|error|status|order|playbook|schedule|run|upload|integration|connected)\b|ზედნადებ|ინტეგრაცი|დაკავშირ|კავშირ|(^|\s)რს(\s|$)|#\d+/i.test(
      text,
    )
  ) {
    return true;
  ...
  const taxTopic =
    /(დღგ|\bvat\b|ხელფას|payroll|ანაზღაურ|დეკლარაცი|declaration|ზედნადებ|waybill|ფაქტურ|invoice|საშემოსავლ|income tax|მცირე\s*მეწარმ|მეწარმ|ბრუნვ|turnover|მოგების გადასახად|profit tax|დივიდენდ|dividend)/i.test(
      text,
    );
  const actionIntent =
