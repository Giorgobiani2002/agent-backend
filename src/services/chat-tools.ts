...
export function chatToolSystemInstruction(): string {
  ...
  return [
    ...
    "Waybill photos ARE supported in chat. If the user asks whether they can upload/send a waybill from a photo/image/scan, say yes and instruct them to attach a clear image in this chat. The backend will OCR/extract the waybill fields, show a preview with buyer TIN, addresses, goods, quantities, prices and warnings, then send to rs.ge only after explicit confirmation. Never say photo/OCR waybill upload is unavailable.",
